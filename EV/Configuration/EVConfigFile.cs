...
namespace cloud.charging.open.EV.Configuration
{
    ...
    /// <summary>
    /// The configuration of this vehicle, in a file beside it.
    /// </summary>
    /// <remarks>
    /// What a vehicle is - how big its battery is, which certificates it is
    /// known by, where it resolves names and reads the time - does not change
    /// between starts and should not have to be repeated at every one. So it is
    /// written down, and the web interface edits the file rather than a copy of
    /// it in memory.
    ///
    ...
    public sealed class EVConfigFile
    {
        ...
        /// </summary>
        public EVConfigFile(String Path)
        {
        ...
        /// </summary>
        public Boolean TryLoad([NotNullWhen(true)] out EVConfiguration?  Configuration,
                                                   out String?                Error)
        {
            ...
            if (!EVConfiguration.TryParse(document, out Configuration, out var problem))
            {
                Error = $"'{Path}': {problem}";
                return false;
            }
