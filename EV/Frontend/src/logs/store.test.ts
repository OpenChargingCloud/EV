 ...
 *
 * What is pinned is what was measured to be wrong: the vehicle was stopped and
 ...
 * "reconnecting ..." over a frozen list, with the vehicle up and running and
...
/** What the vehicle was asked, and what it answered. */
let askedFor: string[] = [];

const vehicleAnswers = (How: 'yes' | 'signed out' | 'not there') => {
...
describe('a stream that stops', () => {
    ...
    it('is left to the browser while the browser is still trying', async () => {
        ...
        try
        {
            ...
            assert.deepEqual(askedFor, [], 'the vehicle was asked about a stream nobody had given up on');
    ...
    it('makes the vehicle be asked why, once the browser has given up', async () => {
        ...
        try
        {
            ...
            assert.deepEqual(askedFor, [], 'the vehicle was asked before anything had settled');

            mock.timers.tick(5_000);
            await Promise.resolve();
            await Promise.resolve();

            assert.equal(askedFor.length, 1, 'nobody asked the vehicle anything');
    ...
    it('is opened again where the vehicle is there and the session still stands', async () => {
    ...
    it('is not opened again when the answer is that nobody is signed in', async () => {
        ...
        try
        {
            ...
            // same refusal.
            assert.equal(Stream.latest, stream, 'a new stream was opened at a vehicle that had refused');
            ...
            assert.equal(askedFor.length, askedOnce,
                         'it went on asking a vehicle that had already said no');
    ...
    it('keeps trying where the vehicle could not answer either', async () => {

        mock.timers.enable({ apis: ['setTimeout'] });

        try
        {
            vehicleAnswers('not there');
            ...
            // A vehicle that is being restarted is away for longer than one
