...
// The browser's copy of the vehicle's log, fed by two things: a snapshot from
// the JSON API and the Server-Sent Events stream. Both carry ids from the same
// counter on the vehicle, and the counter only ever grows - so an entry is
...
/**
 * How many entries the page keeps. The vehicle keeps its own number and
...
/**
 * How long after the stream has given up before the vehicle is asked why.
 *
 * Measured: the vehicle was stopped and started again, which takes every
 ...
 * frozen list for as long as it was left open, with the vehicle up and running
 * and the operator signed out without being told.
 *
 * So when the browser gives up, the vehicle is asked why. A 401 is the answer,
...
/**
 * And how long before asking again, where the vehicle could not answer either.
 *
 * A pace that suits a page somebody has left open in front of a vehicle that
...
export class LogStore {
    ...
    /** Every tag the vehicle has seen, for the filter. */
    ...
    /** How many entries the vehicle keeps. */
    capacity = 0;

    private source:              EventSource | null = null;
    private readonly listeners = new Set<Listener>();

    /** Set while the vehicle is being asked why the stream stopped. */
    ...
    /**
     * Why the stream stopped, asked of the vehicle rather than guessed.
     *
        ...
        this.askAgain = setTimeout(() => {
            ...
            api.auth.me().then(
                ...
                (problem: unknown) => {
                    ...
                    // The vehicle could not answer either, so it is still
    ...
    /** Throw away what this page shows, without touching the vehicle's log. */
    clear(): void {
        this.entries.length = 0;
    ...
    /** The snapshot: what happened up to now, as far back as the vehicle keeps. */
